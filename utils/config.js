...
const devConfig = {
  ...
  API: {
    LOGIN: {
      ...
      INVITE: '/api/couple/bind/accept'  // 使用邀请码邀请用户（已切换至情侣绑定接口）
    },
    USER: {
      INFO: '/api/user/info',
    ...
    CHALLENGE: {
      ...
      UPLOAD: '/api/challenge/upload'        // 上传任务完成照片
    },
    COUPLE: {
      INVITE_CREATE: '/api/couple/invite/create',      // 生成邀请码
    ...
    HEART_WALL: {
      // 项目相关接口
      PROJECTS: '/api/heart-wall/projects',             // 获取用户所有项目、创建项目
      PROJECT_DETAIL: '/api/heart-wall/projects',       // 获取/更新/删除项目详情 (需要拼接projectId)
      // 照片相关接口
      PHOTOS: '/api/heart-wall/photos/upload',          // 上传照片、更新/删除照片 (需要拼接photoId)
      NEXT_POSITION: '/api/heart-wall/next-position'    // 获取下一个可用位置
    },
    TRAJECTORY: {
      // 位置相关接口
      LOCATION_UPDATE: '/api/trajectory/location/update',  // 上传/更新用户位置
