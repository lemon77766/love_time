  ...
  methods: {
    ...
    async loadLetters() {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/xinxiang/history.vue:253", "加载信件失败", error);
        try {
          const localLetters = common_vendor.index.getStorageSync("xinxiang_letters") || [];
          this.letters = localLetters.filter((letter) => letter.status !== "SENT");
        } catch (e) {
          common_vendor.index.__f__("error", "at pages/xinxiang/history.vue:259", "加载本地信件失败", e);
          this.letters = [];
        }
        if (error.statusCode !== 401) {
          common_vendor.index.__f__("warn", "at pages/xinxiang/history.vue:266", "从后端加载信件失败，使用本地数据");
        }
      }
      try {
        ...
        common_vendor.index.__f__("error", "at pages/xinxiang/history.vue:290", "加载已发送信件失败", error);
        if (error.statusCode !== 401) {
          common_vendor.index.__f__("warn", "at pages/xinxiang/history.vue:292", "从后端加载已发送信件失败");
        }
      }
    },
    // 获取状态文本
    ...
    async viewLetter(letter, index) {
      try {
        ...
        common_vendor.index.hideLoading();
        common_vendor.index.__f__("error", "at pages/xinxiang/history.vue:377", "获取信件详情失败", error);
        this.currentLetter = letter;
        this.currentIndex = index;
        this.showDetailModal = true;
      ...
      if (letter && letter.id) {
        try {
          ...
          try {
            ...
            common_vendor.index.__f__("warn", "at pages/xinxiang/history.vue:440", "更新本地存储失败", e);
          }
          common_vendor.index.showToast({ title: "已删除", icon: "success" });
        } catch (error) {
          common_vendor.index.hideLoading();
          common_vendor.index.__f__("error", "at pages/xinxiang/history.vue:446", "删除信件失败:", error);
          common_vendor.index.showToast({
