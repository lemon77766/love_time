...
#### 2.11.1 功能目标
- 基于虚拟人形象模拟情侣吵架场景，帮助双方练习沟通
- 提供多种情景、角色和话题选择

#### 2.11.2 核心功能
- **虚拟形象**：支持自定义双方虚拟形象外观
- **情景剧本**：预设常见矛盾场景，支持自定义话题
- **对话引导**：AI 提供沟通建议与情绪分析
- **复盘总结**：输出沟通要点与建议行动

#### 2.11.3 接口列表
- `GET /api/virtual-argue/scenarios` - 获取吵架剧本列表
- `POST /api/virtual-argue/session` - 创建互动会话
- `POST /api/virtual-argue/message` - 提交对话信息
- `GET /api/virtual-argue/report/{sessionId}` - 获取沟通报告
...
| `pages/virtual-argue/index` | 虚拟吵架 | 虚拟情侣互动入口 |
