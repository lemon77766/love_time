# 历史轨迹查询功能 - 后端接口需求

## 📋 概述

本文档详细说明了历史轨迹查询功能所需的后端接口。前端已实现时间区间选择器，用户可以选择开始日期和结束日期，查询该时间区间内的轨迹点，并在地图上显示和连线。

---

## 🔌 必须实现的接口

### 1. **获取历史轨迹点列表（支持自定义时间区间）**

#### 接口信息
- **请求方法**: `GET`
- **请求地址**: `/api/trajectory/points`
- **请求头**: 需携带 `Authorization: Bearer {token}`

#### 请求参数（Query参数）

支持两种查询方式：

**方式一：使用 period 参数（已支持）**
```
period=7d          // 时间范围：'7d'（7天）或 '30d'（30天）
page=1             // 页码（可选，默认1）
limit=50           // 每页数量（可选，默认50）
```

**方式二：使用自定义时间区间（新增，必须实现）**
```
start_date=2024-01-01    // 开始日期（格式：YYYY-MM-DD）
end_date=2024-01-31      // 结束日期（格式：YYYY-MM-DD）
page=1                   // 页码（可选，默认1）
limit=1000               // 每页数量（可选，默认50，建议支持最大1000）
```

**参数说明**：
- `start_date` 和 `end_date` 必须同时提供，或都不提供
- 如果提供了 `start_date` 和 `end_date`，则忽略 `period` 参数
- `start_date` 必须小于等于 `end_date`
- 日期格式必须为 `YYYY-MM-DD`（例如：`2024-01-01`）
- 查询范围应包含开始日期和结束日期当天的所有轨迹点（时间范围：start_date 00:00:00 至 end_date 23:59:59）

#### 返回数据格式

```json
{
  "success": true,
  "data": {
    "points": [
      {
        "id": 123,
        "user_id": 1,
        "latitude": 39.9042,
        "longitude": 116.4074,
        "address": "北京市朝阳区xxx街道",
        "location_name": "初遇咖啡厅",
        "visit_time": "2024-01-15T10:30:00.000Z",  // ISO时间格式，必填
        "stay_duration": 120,                      // 停留时长（分钟），可选
        "importance_score": 9.5,                  // 重要性评分（0-10），可选
        "visit_count": 1,                          // 到访次数，可选
        "is_manual": false,                        // 是否手动添加，可选
        "description": "第一次见面的地方",          // 描述，可选
        "photos": [                                // 照片列表，可选
          "/path/to/photo1.jpg"
        ]
      }
    ]
  }
}
```

#### 数据字段说明

**轨迹点对象（points数组中的每个元素）**：
- `id` (number, 必填): 轨迹点ID
- `user_id` (number, 必填): 用户ID
- `latitude` (number, 必填): 纬度
- `longitude` (number, 必填): 经度
- `address` (string, 可选): 详细地址
- `location_name` (string, 可选): 地点名称
- `visit_time` (string, 必填): 访问时间，ISO 8601格式（例如：`2024-01-15T10:30:00.000Z`）
- `stay_duration` (number, 可选): 停留时长（分钟）
- `importance_score` (number, 可选): 重要性评分（0-10）
- `visit_count` (number, 可选): 到访次数
- `is_manual` (boolean, 可选): 是否手动添加
- `description` (string, 可选): 描述信息
- `photos` (string[], 可选): 照片URL列表

#### 查询逻辑要求

1. **时间区间查询**：
   - 查询 `visit_time` 在 `[start_date 00:00:00, end_date 23:59:59]` 范围内的轨迹点
   - 必须包含开始日期和结束日期当天的所有轨迹点

2. **数据范围**：
   - 只返回当前用户及其配对用户的轨迹点（双方轨迹点）
   - 如果用户未绑定配对关系，只返回当前用户的轨迹点

3. **排序**：
   - 轨迹点按 `visit_time` 升序排序（时间从早到晚）
   - 前端会根据这个顺序连接轨迹线

4. **分页**：
   - 支持分页查询，但建议前端一次性加载所有数据（limit=1000）以便完整显示轨迹线
   - 如果数据量很大，可以考虑后端分页，但前端需要处理多页数据合并

5. **性能优化建议**：
   - 如果时间区间很大（如超过1年），建议限制查询范围或提醒用户缩小时间区间
   - 可以考虑对轨迹点进行去重或聚合（相同地点多次访问，只返回重要的一次）

#### 错误处理

**参数错误**：
```json
{
  "success": false,
  "message": "开始日期不能晚于结束日期",
  "code": "INVALID_PARAMS"
}
```

**时间区间过大**：
```json
{
  "success": false,
  "message": "查询时间区间不能超过365天",
  "code": "DATE_RANGE_TOO_LARGE"
}
```

**无数据**：
```json
{
  "success": true,
  "data": {
    "points": []
  }
}
```

---

## 📊 数据模型建议

### 轨迹点表结构（trajectory_points）

```sql
CREATE TABLE trajectory_points (
  id BIGINT PRIMARY KEY AUTO_INCREMENT,
  user_id BIGINT NOT NULL,
  latitude DECIMAL(10, 8) NOT NULL,
  longitude DECIMAL(11, 8) NOT NULL,
  address VARCHAR(255),
  location_name VARCHAR(100),
  visit_time DATETIME NOT NULL,           -- 访问时间
  stay_duration INT,                      -- 停留时长（分钟）
  importance_score DECIMAL(3, 1),         -- 重要性评分（0-10）
  visit_count INT DEFAULT 1,             -- 到访次数
  is_manual BOOLEAN DEFAULT FALSE,        -- 是否手动添加
  description TEXT,                       -- 描述
  photos JSON,                            -- 照片列表（JSON数组）
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  INDEX idx_user_visit_time (user_id, visit_time),
  INDEX idx_visit_time (visit_time)
);
```

### 查询SQL示例

```sql
-- 查询指定时间区间内的轨迹点
SELECT 
  id,
  user_id,
  latitude,
  longitude,
  address,
  location_name,
  visit_time,
  stay_duration,
  importance_score,
  visit_count,
  is_manual,
  description,
  photos
FROM trajectory_points
WHERE 
  user_id IN (?, ?)  -- 当前用户ID和配对用户ID
  AND visit_time >= ?  -- start_date 00:00:00
  AND visit_time <= ?  -- end_date 23:59:59
ORDER BY visit_time ASC
LIMIT ? OFFSET ?;
```

---

## 🎯 实现优先级

### 高优先级（必须实现）
1. ✅ 支持 `start_date` 和 `end_date` 参数
2. ✅ 返回轨迹点列表，包含所有必需字段
3. ✅ 按 `visit_time` 排序

### 中优先级（建议实现）
1. ⚠️ 支持分页查询（处理大数据量）
2. ⚠️ 时间区间范围限制和验证

### 低优先级（可选）
1. 📝 轨迹点去重或聚合
2. 📝 性能优化（缓存、索引等）

---

## 🔍 测试用例

### 测试用例1：正常查询
```
GET /api/trajectory/points?start_date=2024-01-01&end_date=2024-01-31&limit=1000
```
**预期结果**：返回2024年1月1日至31日之间的所有轨迹点

### 测试用例2：单日查询
```
GET /api/trajectory/points?start_date=2024-01-15&end_date=2024-01-15&limit=1000
```
**预期结果**：返回2024年1月15日当天的所有轨迹点

### 测试用例3：参数错误
```
GET /api/trajectory/points?start_date=2024-01-31&end_date=2024-01-01&limit=1000
```
**预期结果**：返回错误："开始日期不能晚于结束日期"

### 测试用例4：无数据
```
GET /api/trajectory/points?start_date=2020-01-01&end_date=2020-01-31&limit=1000
```
**预期结果**：返回空数组 `points: []`

---

## 📝 注意事项

1. **时区处理**：确保 `visit_time` 的时区处理正确，建议统一使用UTC时间存储，前端根据时区转换显示

2. **数据完整性**：确保返回的轨迹点数据包含前端所需的所有字段，避免前端报错

3. **性能考虑**：
   - 如果时间区间很大，建议限制查询范围（如最多365天）
   - 为 `user_id` 和 `visit_time` 字段建立索引以提高查询性能

4. **兼容性**：保持对 `period` 参数的支持（向后兼容），同时支持新的 `start_date`/`end_date` 参数

---

## ✅ 前端已实现功能

- ✅ 时间区间选择器（开始日期、结束日期）
- ✅ 查询按钮和加载状态
- ✅ 历史轨迹点地图显示
- ✅ 轨迹点按时间顺序连线
- ✅ 点击轨迹点查看详情
- ✅ 清除历史轨迹功能

---

## 📞 联系

如有问题或需要澄清，请参考前端代码：
- API调用：`api/trajectory.js` 中的 `getTrajectoryPoints` 函数
- 页面实现：`pages/trajectory/index.vue`

